...
type MyPageHighlightProps = {
  to?: string;
};

export default function MyPageHighlight({ to = '/me' }: MyPageHighlightProps) {
  ...
  return (
    <Link to={to} aria-label="マイページへ" title="マイページへ" style={linkStyle}>
      <div style={cardStyle}>
        <div style={titleRowStyle}>
          <svg width={20} height={20} viewBox="0 0 640 640" aria-hidden>
          ...
          <h2 style={{ margin: 0, color: 'var(--color-text-secondary)' }}>マイページ</h2>
        </div>
        <p style={{ margin: 0, color: 'var(--color-text-tertiary)' }}>お気に入りや履歴を確認</p>
      </div>
    </Link>
  );
}
