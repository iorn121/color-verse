...
const IDENTITY: Mat3 = [1, 0, 0, 0, 1, 0, 0, 0, 1] as const;

const clampUnit = (x: number): number => (x < 0 ? 0 : x > 1 ? 1 : x);
const clampByte = (x: number): number => clamp(Math.round(x), 0, 255);

const PROTAN_MATS: Mat3[] = [
  // 0.0
  IDENTITY,
  // 0.1
...
function getSeverityMatrix(type: CvdType, severity: number): Mat3 {
  if (type === 'common') return IDENTITY;
  const s = clampUnit(severity);
  const idx = s * 10;
  const lo = Math.floor(idx);
  const hi = Math.ceil(idx);
  const t = idx - lo;
  const arr =
    type === 'protanopia' ? PROTAN_MATS : type === 'deuteranopia' ? DEUTAN_MATS : TRITAN_MATS;
  return lo === hi ? (arr[lo] as Mat3) : lerpMat(arr[lo] as Mat3, arr[hi] as Mat3, t);
}

function transformRgbForCvd({ r, g, b }: Rgb, type: CvdType, severity: number): Rgb {
  const mat = getSeverityMatrix(type, severity);
...
function applyCvdToImageData(data: Uint8ClampedArray, type: CvdType, severity: number) {
  if (type === 'common') {
    return;
  }
  for (let i = 0; i < data.length; i += 4) {
    const rgb = { r: data[i], g: data[i + 1], b: data[i + 2] };
    const t = transformRgbForCvd(rgb, type, severity);
    data[i] = t.r;
    data[i + 1] = t.g;
    data[i + 2] = t.b;
  }
...
export function useCameraCvd() {
  ...
  const loop = useCameraLoop({
    ...
    onFrame: ({ ctx, video, canvas }) => {
      ...
      applyCvdToImageData(img.data, cvd, severity);
